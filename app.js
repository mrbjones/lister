...
response.write("<div ng-bind=\"myHTML\"></div>")
response.write("<input type=button value='Press Me!' ng-click=\"update(cars)\">")
...
response.write("<script>  angular.module('getter', ['ngSanitize'])    .controller('ListCarz', ['$scope', '$http', function($scope,$http) {$scope.update = function(cars,$scope)  { $http({ url: 'http://btjweb1.uswest.appfog.ctl.io/app.js',    method: \"GET\",    params: {o: 'g'}}).success(function(data, status, headers, config, cars, $scope) { alert(data); $scope.myHTML = data });   };     }]);")
response.write("</script>")

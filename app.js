...
response.write(" $scope.update = function(carz)  { $http({ url: 'http://btjweb4.uswest.appfog.ctl.io/app.js',    method: \"GET\",    params: {o: 'p',key: carz.key, make: carz.make, model: carz.model, color: carz.color, year:carz.year}}).success(function(data, status, headers, config) {$scope.listcars(); $scope.$apply; $scope.carz.key.value=''; });   };    ")

...
response.write("<script>  angular.module('getter', ['ngSanitize'])    .controller('ListCarz', ['$scope', '$http', function($scope,$http) {$scope.update=function($scope)  { $http({ url: 'http://btjweb1.uswest.appfog.ctl.io/app.js',    method: \"GET\",    params: {o: 'g'}}).success(function(data, status, headers, config, $scope) { alert(data); $scope.myHTML = data });   };     }]);")
response.write("</script>")

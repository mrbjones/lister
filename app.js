...
response.write(" $scope.deleter= function(car){ if (window.confirm(\"Really Delete?\")) {  $http({ url: 'http://btjweb3.uswest.appfog.ctl.io/app.js',    method: \"GET\",    params: {o: 'd', key: car.path.key}}).success(function(data, status, headers, config) { angular.element(document.getElementById(\"main\")).scope(); scope.$apply  });   }; };  };     }]);")
